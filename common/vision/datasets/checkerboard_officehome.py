...
class CheckerboardOfficeHome():
    ...
    def __init__(self,
                ...
                use_mixed_split: Optional[bool] = True,
                train_val_test_split: Optional[Tuple[float]] = (0.5, 0.25, 0.25),
                styles_per_cat: Optional[int] = 2,
        ...
        self.generate_image_list(root, use_mixed_split, styles_per_cat, balance_domains)
        datasets = []
    ...
    def generate_image_list(
        ...
        assert use_mixed_split or not styles_per_cat <= 1
        ...
        if balance_domains:
            style_combs = list(combinations(style_indices, styles_per_cat))
                    ...
                    if filename.endswith(".jpg"):
                        ...
                        if balance_domains:
                            if style_index in bal_styles_per_cat[cat_index]:
                                if style_index == bal_styles_per_cat[cat_index][0]:
        ...
        if use_mixed_split:
            non_novel = train + val_and_test
            random.shuffle(non_novel)
            num_train = int(len(non_novel) * self.train_split)
            num_val = int(len(non_novel) * self.val_split)
            train_list = "".join(non_novel[:num_train])
            val_list = "".join(non_novel[num_train:(num_train + num_val)])
            test_list = "".join(non_novel[(num_train + num_val):])
        else:
