...
class ImageList(datasets.VisionDataset):
    ...
    def__init__(self, root: str, classes: List[str], data_list_file: Optional[str] = None, data_list_files: Optional[List[str]] = None, transform: Optional[Callable] = None, target_transform: Optional[Callable] = None):
        ...
        self.data_list_files = data_list_files
        if not self.data_list_files:
            self.data_list_files = [self.data_list_file]
        self.samples = self.parse_data_files(self.data_list_files)
    ...
    def parse_data_files(self, file_names: List[str]) -> List[Tuple[str, int]]:
