...
class Classifier(nn.Module):
    ...
    def get_parameters(self):
        params = [
            ...
            {"params": self.head.parameters(), "lr_mult": 10.},
        ]
        return params
...
class AdversarialClassifier(nn.Module):
    def __init__(self, in_features, num_classes, bottleneck_dim):
        ...
        self.head = ClassifierHead(in_features, num_classes, use_bottleneck=True, bottleneck_dim=bottleneck_dim)
    ...
    def get_parameters(self):
        params = [{"params": self.head.parameters(), "lr_mult": 10.}]
        return params
...
class MarginDisparityDiscrepancyLoss(nn.Module):
    ...
    def forward(self, y_s, f_s, y_t, f_t):
        ...
        y_t_adv = self.adversarial_classifier(f_t)
...
class MarginDisparityDiscrepancy(nn.Module):
    ...
    def forward(self, y_s, y_s_adv, y_t, y_t_adv):
        ...
        return self.margin * F.nll_loss(F.log_softmax(y_s_adv, dim=1), prediction_s, reduction=self.reduction) \
               + F.nll_loss(torch.log(1. - F.softmax(y_t_adv, dim=1)), prediction_t, reduction=self.reduction)
