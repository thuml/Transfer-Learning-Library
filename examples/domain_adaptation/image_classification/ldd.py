...
def main(args: argparse.Namespace):
    ...
    classifier = ImageClassifier(backbone, num_classes, bottleneck_dim=args.bottleneck_dim,
                                 width=args.bottleneck_dim, pool_layer=pool_layer, finetune=args.pretrain_finetune).to(device)
    mdd = MarginDisparityDiscrepancy(args.margin).to(device)

    # pretrain on the source domain to decrease the hypothesis space
    if not osp.exists(logger.get_checkpoint_path('pretrain')):
        print("start pretraining on the source domain")
        pretrain_optimizer = SGD(classifier.get_parameters(), args.pretrain_lr, momentum=args.momentum,
                                 weight_decay=args.wd, nesterov=True)
        pretrain_lr_scheduler = LambdaLR(pretrain_optimizer,
                                         lambda x: args.pretrain_lr * (1. + args.lr_gamma * float(x)) ** (
                                             -args.lr_decay))
        for epoch in range(args.pretrain_epochs):
            print(pretrain_lr_scheduler.get_lr())
            pretrain(train_source_iter, classifier, pretrain_optimizer, pretrain_lr_scheduler, epoch, args)
            utils.validate(val_loader, classifier, args, device)
            torch.save(classifier.state_dict(), logger.get_checkpoint_path('pretrain'))
    else:
        print("loading pretrained model from {}".format(logger.get_checkpoint_path('pretrain')))
        classifier.load_state_dict(torch.load(logger.get_checkpoint_path('pretrain')))
...
def pretrain(train_source_iter, model, optimizer, lr_scheduler, epoch: int, args: argparse.Namespace):
    ...
    for i in range(args.iters_per_epoch):
        ...
        acc_s.update(cls_acc, x_s.size(0))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain-lr', default=0.01, type=float,
                        help='pretrain initial learning rate')
    parser.add_argument('--pretrain-finetune', action='store_true',
                        help='whether use 10x times learning rate during pretrain phase')
