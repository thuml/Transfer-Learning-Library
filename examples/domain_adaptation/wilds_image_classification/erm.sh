CUDA_VISIBLE_DEVICES=3 python erm.py data/wilds -d "fmow" --aa "v0" --arch "densenet121" \
  --lr 0.1 --opt-level O1 --deterministic --vflip 0.5 --log logs/erm/fmow/lr_0_1_aa_v0_densenet121


CUDA_VISIBLE_DEVICES=1 python erm.py data/wilds -d "iwildcam" --unlabeled-list "extra_unlabeled" --lr 0.3 --opt-level O1 \
