...
def main(args: argparse.Namespace):
    ...
    # create model
    print("=> using pre-trained models '{}'".format(args.archs))

    backbone = utils.build_homogeneous_model(args.archs, lite=args.lite)
    classifier = Classifier(backbone, num_classes, finetune=args.finetune).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a', '--archs', nargs='+',
                        choices=['imageNet', 'moco', 'maskrcnn', 'deeplab', 'keyPoint'], 
                        help='homogeneous models pretrained by different methods')
    parser.add_argument('--finetune', action='store_true', help='whether use 10x smaller lr for backbone')
    parser.add_argument("--lite", action='store_true',
                        help="whether to use lite version of zoo-tuning")
    ...
    parser.add_argument('--lr-decay-epochs', type=int, default=(15, 22), nargs='+', help='epochs to decay lr')
