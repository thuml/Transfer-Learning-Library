...
class MDDTrainer(nn.Module):
    def __init__(self, classifier, cls_loss_fn, mdd_loss_fn, trade_off):
        super(MDDTrainer, self).__init__()
        self.classifier = classifier
        self.mdd_loss_fn = mdd_loss_fn
        self.cls_loss_fn = cls_loss_fn
        self.trade_off = trade_off

    def forward(self, x_s, x_t, labels_s, labels_t):
        x = torch.cat((x_t, x_s), dim=0)
        outputs, outputs_adv = self.classifier(x)
        y_t, y_s = outputs.chunk(2, dim=0)
        y_t_adv, y_s_adv = outputs_adv.chunk(2, dim=0)

        # compute cross entropy loss on source domain
        cls_loss = self.cls_loss_fn(y_s, labels_s)
        # compute margin disparity discrepancy between domains
        transfer_loss = self.mdd_loss_fn(y_s, y_s_adv, y_t, y_t_adv)
        loss = cls_loss + transfer_loss * self.trade_off
        cls_acc = accuracy(y_s, labels_s)[0]
        tgt_acc = accuracy(y_t, labels_t)[0]
        return loss, cls_loss, transfer_loss, cls_acc, tgt_acc
...
def main(args):
    ...
    val_loader = DataLoader(val_dataset, batch_size=args.batch_size*2, shuffle=False, num_workers=args.workers)
    ...
    trainer = MDDTrainer(classifier, nn.CrossEntropyLoss(), mdd, args.trade_off)
    ...
    for epoch in range(args.epochs):
        # train for one epoch
        train(train_source_iter, train_target_iter, trainer, optimizer,
...
def train(train_source_iter, train_target_iter, trainer, optimizer,
          lr_scheduler, epoch, args):
    ...
    for i in range(args.iters_per_epoch):
        ...
        results = trainer(x_s, x_t, labels_s, labels_t)
        results = (r.mean() for r in results)
        loss, cls_loss, transfer_loss, cls_acc, tgt_acc = results
        ...
        loss.backward()
        optimizer.step()
        trainer.module.classifier.step()
