...
__all__ = ['Classifier', 'Classifier_with_heads']
...
class Classifier_with_heads(nn.Module):
    """A generic Classifier class for domain adaptation.
    ...
    def __init__(self, backbone: nn.Module, num_classes: int,  source_head: Optional[nn.Module] = None, finetune=True):
        super(Classifier_with_heads, self).__init__()
        self.backbone = backbone
        self.num_classes = num_classes
    ...
    def get_parameters(self) -> List[Dict]:
        ...
        params = [
            {"params": self.backbone.parameters(), "lr": 0.1 if self.finetune else 1.},
            {"params": self.source_head.parameters(), "lr": 0.1},
            {"params": self.head.parameters(), "lr": 1.},
        ]
        return params
