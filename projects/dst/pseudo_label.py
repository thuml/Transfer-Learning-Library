...
class BNController:
    """Store current BN statistics and recover these values later. Adopted from
    https://github.com/TorchSSL/TorchSSL/blob/main/train_utils.py
    """

    def __init__(self):
        self.backup = {}

    def store_bn(self, model):
        assert self.backup == {}
...
def train(labeled_train_loader: DataLoader, unlabeled_train_loader: DataLoader, val_loader: DataLoader, model: DDP,
    ...
    for global_step, (x_l, labels_l), (x_u, _) in \
        ...
        bn_controller.store_bn(model)
        y_u = model(x_u)
        bn_controller.recover_bn(model)
        self_training_loss, mask, _ = self_training_criterion(y_u, y_u)

        warmup_factor = min(1.0, global_step / (args.warmup_length * args.train_iterations))
        self_training_loss = args.trade_off_self_training * warmup_factor * self_training_loss
        ...
        if global_step % args.print_freq == 0:
            ...
            print('iteration: [{0}]\t'
                ...
                batch_time=batch_time, cls_loss=cls_losses, self_training_loss=self_training_losses, loss=losses,
                cls_accs=cls_accs, ratio=pseudo_label_ratios))
            ...
            if args.local_rank == 0:
                ...
                torchvision.utils.save_image(
                    x_u,
                    os.path.join(args.log, 'visualize', 'unlabeled-data.jpg'),
                    padding=0,
                    normalize=True)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Pseudo Label')
    ...
    parser.add_argument('--train-iterations', default=400000, type=int, metavar='N',
                        help='number of total iterations to run')
    parser.add_argument('--warmup-length', default=0.4, type=float,
                        help='warmup length of self training loss')
    ...
    parser.add_argument('-ub', '--unlabeled-batch-size', default=64, type=int,
